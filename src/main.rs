...
fn main() {
    ...
    */
    ...
    let mary_customer = bob_customer.clone();

    bob_customer.person().set_first_name("Bob".to_string());
    bob_customer.person().set_last_name("Bob".to_string());
    bob_customer.set_customer_num(1);

    test_customers.push(bob_customer);
    test_customers.push(mary_customer);
    main_menu::MainMenu::bank_manager(test_customers);

    //main_menu::MainMenu::main_menu(test_customers);
